...
const Task = ({task, onRemove, list,onEdit, onCompleteTask}) => {
    const onChangeCheckbox = (e) => {
        onCompleteTask(list.id, task.id, e.target.checked);
    }
    return (
        <div key = {task.id} className="tasks-items-blok">
            <input readOnly value={task.text} />

            <div className="tasks-items-blok-actions">
                <div className='checkbox'>
                    <input id = {`task-${task.id}`} type="checkbox" onChange={onChangeCheckbox}  checked = {task.completed} />
                    <label htmlFor={`task-${task.id}`}>
                        <svg width="15" height="12" viewBox="0 0 11 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9.29999 1.20001L3.79999 6.70001L1.29999 4.20001" stroke="red" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                    </label>
                </div>

                <div onClick={() => onEdit(list.id, task)}>
                    <svg
                        width="15"
                        height="15"
                        viewBox="0 0 15 15"
                ...
                <div onClick={() => onRemove(list, task)}>
                    <svg
                        width="11"
                        height="11"
                        viewBox="0 0 11 11"
